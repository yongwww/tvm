...
tuning_option = {
    ...
    'measure_option': autotvm.measure_option(
        ...
        runner=autotvm.LocalRunner(number=2, repeat=1,
                                   min_repeat_ms=10),
    ),
}
...
def tune_graph(graph, dshape, records, opt_sch_file, use_DP=True):
    ...
    executor.benchmark_layout_transform(min_exec_num=20)
    executor.run()
...
def tune_and_evaluate(tuning_opt):
    ...
    if rt == 'graph_runtime':
        print("Autotuning with Graph Runtime")
        with autotvm.FallbackContext():#.apply_graph_best(graph_opt_sch_file):
            print("Compile...")
        ...
        print("Autotuning with Virtual Machine")

        data_tvm = np.random.uniform(size=data_shape).astype(dtype)
        ctx = tvm.cpu()

        ex = relay.create_executor('vm', mod=mod, ctx=ctx)
        result = ex.evaluate()(data_tvm, **params).asnumpy().astype(dtype)
        ...
        with autotvm.apply_graph_best(graph_opt_sch_file):
            ...
            #module = relay.Module()
            #module["main"] = mod
            #print("mod['main']:  {}".format(mod["main"]))
            """
            compiler = relay.vm.VMCompiler()
            vm = compiler.compile(mod, target)
            vm.init(ctx)
            vm.invoke("main", *data_tvm)
            """
    elif rt == 'vm_autotvm': # VM with autotvm
        print("#### AutoTVM with Virtual Machine")
        ...
        # 2) Virtual Machine
        # compile kernels with graph-level best records
        with autotvm.apply_graph_best(graph_opt_sch_file): # autotvm.FallbackContext()
            print("Compile...")
            #print("mod previous:\n{}".format(mod))
            with relay.build_config(opt_level=3):
                #new_mod, new_params = relay.build_vm(mod, target=target, params=params)
                ex = relay.create_executor('vm', mod=mod, ctx=ctx)
                result = ex.evaluate()(data_tvm, **params).asnumpy().astype(dtype)
