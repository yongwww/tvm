...
class Module:
    ...
    def main(
        ...
        embedding: R.Tensor((2, 128), dtype="float32"),
        ...
        with R.dataflow():
            ...
            matmul146: R.Tensor((1, 1, 2, 128), dtype="float32") = R.matmul(
                subtract56, embedding, out_dtype="void"
            )
            ...
            R.output(matmul146, mul128, debugging)
        return (matmul146, mul128, debugging)
...
if __name__ == "__main__":
    ...
    out1 = test_matmul_cutlass(inputs, apply_cutlass=False)
    out2 = test_matmul_cutlass(inputs, apply_cutlass=True)

    for i in range(len(out1)):
        tvm.testing.assert_allclose(out1[i].asnumpy(), out2[i].asnumpy(), rtol=1e-1, atol=1e-1)
        print("output[{}] results match".format(i))
