...
# Questions:
# 1) relax.build to accept list/dict of targets
# 2) call_tir_rewrite to identify the device_index
...
def run_cpu_gpu(mod):
    ...
    mod.show()
    ex = relax.build(mod, target_cpu)  # => [target_cpu, target_gpu]
    ...
    ####### Planned changes
    # c0: Add partition to tir_mod in _vmlink in vm_build.py, group the tir_mod by target
    # c1: Limit the number and ordering of devices during runtime to match the number and
    #     ordering of vdevices defined in the model global_info. (Do we want to remove the un-used vdevice in global_info?)
    # c2: Modify the call_tir_rewrite pass to module pass from function pass, when call into alloc_tensor,
    #     retrieve the vdevice info from tensorstructinfo, and figure out the corresponding index in vdevice list
    ...
    class Example:
        ...
        def foo(
            ...
            with R.dataflow():
                ...
                gv: R.Tensor((2, 4), "float32", "cuda") = R.matmul(lv1, z)
                R.output(gv)
            return gv
